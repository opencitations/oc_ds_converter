...
class RorIdentifierManagerTest(unittest.TestCase):
    ...
    def tearDown(self):
        if exists("storage"):
            shutil.rmtree("storage")
        test_db = os.path.join(self.test_dir, "database.db")
        if exists(test_db):
            os.remove(test_db)
    ...
    def test_ror_default(self):
        # Default storage manager: RedisStorageManager
        rm = RORManager()
        self.assertTrue(rm.is_valid(self.valid_ror_1))
        self.assertTrue(rm.is_valid(self.valid_ror_2))
        self.assertFalse(rm.is_valid(self.invalid_ror_1))
        ...
        validated_ids = {rm.normalise(x, include_prefix=True) for x in validated_ids}
        all_ids_stored = rm.storage_manager.get_all_keys()
        self.assertEqual(validated_ids, all_ids_stored)
        rm.storage_manager.delete_storage()
        self.assertEqual(rm.storage_manager.get_all_keys(), set())
    ...
    def test_ror_redis_file_api(self):
        ...
        for id in to_insert:
            norm_id = sql_file.normalise(id, include_prefix=True)
            sql_file.is_valid(norm_id)
            sql_file.storage_manager.set_value(norm_id, sql_file.is_valid(norm_id))
