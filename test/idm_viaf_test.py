...
class viafIdentifierManagerTest(unittest.TestCase):
    ...
    def test_exists(self):
        ...
        with self.subTest(msg="get_extra_info=False, allow_extra_api='None'"):
            viaf_manager = ViafManager()
            output = viaf_manager.exists(self.invalid_viaf_1, get_extra_info=False, allow_extra_api=None)
            expected_output = False
            self.assertEqual(output, expected_output)
        with self.subTest(msg="get_extra_info=True, allow_extra_api='None'"):
            viaf_manager = ViafManager()
            output = viaf_manager.exists(self.invalid_viaf_1, get_extra_info=True, allow_extra_api=None)
            expected_output = (False, {'id': viaf_manager.normalise(self.invalid_viaf_1), 'valid': False})
            self.assertEqual(output, expected_output)
